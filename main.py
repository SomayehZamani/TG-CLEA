...
def train():
    ...
    def train_model(epoch, G1_flag, pretrain, temp=1, batch_size=256, alternative_train_batch=100, temp0 = Config().temp_min):
        ...
        temp = temp
        batch_num = 0
        #print("  ****   train dataset   ", len(TRAIN_DATASET[0]))
        for batchid, (batch_userid, batch_input_seq, batch_target, batch_history, batch_neg_items, org_seq) in enumerate(
                get_batch_TRAIN_DATASET(TRAIN_DATASET, batch_size)):

            #print("batch_userid  ", batch_userid.shape)
            #print("batch_input_seq  ", batch_input_seq.shape)
            #print("batch_target  ", batch_target.shape)
            #print("batch_history  ", batch_history.shape)
            #print("batch_neg_items  ", batch_neg_items.shape)
            #print("org_seq    ", len(org_seq))
            #edge_index = get_edge_matrix(org_seq)
            #print("pre  ", batch_input_seq.shape)
            edge_index = get_edge_matrix_2(batch_input_seq)
            #print("after   ", edge_index.shape)
            # print("------")
            # print("batch sample   ", batch_input_seq[0,:])
            # print("------")
            batch_num += 1
            if G1_flag == 0:
            ...
            for batchid, (batch_userid, batch_input_seq, pad_batch_target_bsk, batch_history) in enumerate(
                ...
                if 1:
                    ...
                    for bid, pad_target_bsk in enumerate(pad_batch_target_bsk):  ##
                        ...
                        #print("pre val    ", batch_input_seq.shape)
                        edge_index = get_edge_matrix_2(input_tensor_expand)
                        #print("after val   ", edge_index.shape)
                        edge_index = torch.from_numpy(edge_index)
                        edge_index = edge_index.to(device)
                        ####
                        #print("1s   ", input_tensor_expand.shape)
                        #print("2s   ", target_items_tensor.shape)
                        rest_ratio,repeat_ratio, prob = NB(temp, uid_tensor_expand, input_tensor_expand, target_items_tensor, weights,
                                            history_tensor_expand, edge_index=edge_index,
                                            neg_set_tensor=None, train=False, G1flag=G1_flag, pretrain=pretrain, sd2=sd)
                        
                        # rest_ratio,repeat_ratio, prob = NB(temp, uid_tensor_expand, batch_input_seq, target_items_tensor, weights,
                        #                      history_tensor_expand, edge_index=edge_index,
                        #                      neg_set_tensor=None, train=False, G1flag=G1_flag, pretrain=pretrain, sd2=sd)
        ...
        with torch.no_grad():
            for batchid, (batch_userid, batch_input_seq, pad_batch_target_bsk, batch_history) in enumerate(
                ...
                for bid, pad_target_bsk in enumerate(pad_batch_target_bsk):  ##
                    ...
                    ##
                    edge_index = get_edge_matrix_2(input_tensor_expand)
                    #print("after val dream   ", edge_index.shape)
                    edge_index = torch.from_numpy(edge_index)
                    edge_index = edge_index.to(device)
                    ##
                    rest_ratio,repeat_ratio, prob = NB(temp, uid_tensor_expand, input_tensor_expand, target_items_tensor, weights,
        ...
        with torch.no_grad():
            for batchid, (batch_userid, batch_input_seq, pad_batch_target_bsk, batch_history) in enumerate(
                ...
                for bid, pad_target_bsk in enumerate(pad_batch_target_bsk):  ##
                    ...
                    # print("pre val dream   ", batch_input_seq.shape)
                    # edge_index = get_edge_matrix_2(batch_input_seq)
                    # print("after val dream   ", edge_index.shape)
                    # edge_index = torch.from_numpy(edge_index)
                    # edge_index = edge_index.to(device)
                    ####
                    #print("pre val dream   ", input_tensor_expand.shape)
                    edge_index = get_edge_matrix_2(input_tensor_expand)
                    #print("after val dream   ", edge_index.shape)
                    edge_index = torch.from_numpy(edge_index)
                    edge_index = edge_index.to(device)
                    ####
                    #print("1s dream   ", input_tensor_expand.shape)
                    #print("2s dream   ", target_items_tensor.shape)
