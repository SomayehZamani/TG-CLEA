...
class NBModel(nn.Module):  #
    ...
    # profile
    def forward(self, T, uid, input_seq_tensor, tar_b_tensor, weight, history, neg_set_tensor=None, train=True, edge_index=None,
        ...
        if ((self.G1_flag == 0) & (pretrain == 0)):  #
            ...
            #print("nb   inp  ", input_seq_tensor.shape)
            real_generate_embedding1 = self.G2(self.filter_basket, input_seq_tensor,uid, edge_index=edge_index)  # K*H
            ...
            real_generate_embedding1 = self.G2(self.filter_basket[:, :, 0], input_seq_tensor,uid, edge_index=edge_index)
            ...
            if train == True:
                ...
                rest_generate_embedding1 = self.G2(self.filter_basket[:, :, 1], input_seq_tensor,uid, edge_index=edge_index)
                ...
                whole_generate_embedding1 = self.G2(self.whole_basket, input_seq_tensor,uid, edge_index=edge_index)  # K*H
            ...
            test_generate_embedding1 = self.G2(test_basket, input_seq_tensor,uid, edge_index=edge_index)
...
class GAT(torch.nn.Module):
    def __init__(self, hidden_size, heads):
        ...
        self.conv1 = GATConv(self.hid, self.hid, heads=self.in_head, dropout=0.3)
        self.conv2 = GATConv(self.hid, self.hid, concat=False,
                             heads=self.out_head, dropout=0.3)

    def forward(self, data, adj):
        
        #print("x1   ", x.float().dtype)
        #print("x2   ", edge_index.int().dtype)
        #print("x1   ", x.shape)
        #print("x2   ", edge_index.shape)
        x = self.conv1(data, adj)
        #x = F.elu(x)
...
class Generator2(nn.Module):
    def __init__(self, config, device, dropout_p=0.2):
        ...
        #self.gru1 = nn.LSTM(self.gru_hidden_size,
        ...
        self.in_head = 1
        #self.gconv1 = GATConv(self.gru_hidden_size, self.gru_hidden_size, heads=self.in_head, dropout=0.3)
        self.gconv1 = GAT(self.gru_hidden_size, heads=self.in_head)


    # profile
    def forward(self, filter_basket_prob, input_seq_tensor,uid = None, edge_index=None):
        '''
        ...
        #print(" --   0   ", input_embeddings1.shape)

        #print("gen2 0  ", input_embeddings1.shape)
        ...
        #print(" --   0 1  ", input_embeddings1.shape)
        #print("gen2 1  ", input_embeddings_f1.shape)
        ...
        #print(" --   0 2  ", input_embeddings1.shape)
        #print("gen2 2  ", input_embeddings_f1.shape)
        ...
        if self.basket_pool_type == 'avg':
            ...
            #print(" -- 44 0  ", input_embeddings_f1.shape)
            basket_embedding1 = (torch.sum(input_embeddings_f1, dim=2) / (
                    filtered_tensor.sum(dim=2) + 1e-10))  # K*basket_num*H
            
            #print(" -- 44   ", basket_embedding1.shape)
            basket_embedding2 = torch.sum(basket_embedding1, dim=1)
            #print(" -- 44 011  ", basket_embedding1.shape)
        else:
            ...
            #print(" -- 44 1  ", basket_embedding1.shape)
        ...
        #print("sort embeds    ", sort_embed_input1.shape)
        ...
        #print(" -- 1   ", sort_embed_input1.shape)
        #print(" -- 2   ", edge_index.shape)
        #print(" --  3  ", input_seq_tensor.shape)
        #print(" -- 7   ", basket_embedding2.shape)

        out_pack1 = self.gconv1(basket_embedding2, edge_index.int())
        # try:
        #     #print(" -- 2   ", edge_index.shape)
        #     #print(" -- 7   ", basket_embedding2.shape)
        #     out_pack1 = self.gconv1(basket_embedding2, edge_index.int())
        # except:
        #     #print(" -- 2 exp  ", edge_index.shape)
        #     #print(" -- 7 exp  ", basket_embedding2.shape)
        #     pass
