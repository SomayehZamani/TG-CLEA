...
def get_edge_matrix(all_sequences):
    # all_nodes = []
    # for i in range(len(seq)):
    #     all_nodes.extend(seq[i])
    # ##
    # ret = []
    ret = np.array([[0],[0]])
    for s in range(len(all_sequences)):
        seq = all_sequences[s]
        edges = np.array([[0],[0]])
        for i in range(len(seq)-1):
            for j in range(len(seq[i])):
                for k in range(len(seq[i+1])):
                    new_link = [[seq[i][j]],[seq[i+1][k]]]
                    new_link = np.array(new_link)
                    #edges = np.concatenate((edges, new_link), axis=1)
                    ret = np.concatenate((ret, new_link), axis=1)
        #print("edges   ", edges.shape)
        #ret.append(edges)
    ret = np.array(ret)
    ret = np.unique(ret, axis=1)
    #print("get edge index ret   ", len(ret))
    print("get edge index ret   ", ret.shape)
    return ret

def get_edge_matrix_2(X_inp):
    X = X_inp.cpu().numpy() #X.detach().numpy()
    #print("X   ", X.shape)
    unq_x = np.unique(X,axis=0)
    #print("unq shape   ", unq_x.shape)
    edges = np.zeros((2,len(X)))
    #print(edges.shape)
    for i in range(len(X)-1):
        #if i % 100 == 0:
        #    print(i//100, end=" ")
        #print("a   ", np.where(np.all(unq_x == X[i,:].reshape(1,-1),axis=1)))
        i_ind = np.where((unq_x == X[i,:]).all(axis=1))[0]
        j_ind = np.where((unq_x == X[i+1,:]).all(axis=1))[0]
        # print("iind  ", i_ind[0])
        # print("jind  ", j_ind[0])
        edges[0,i] = i_ind[0]
        edges[1,i] = j_ind[0]
...
def get_dataset(input_dir, max_basket_size,max_basket_num,neg_ratio,history = 0,next_k = 1):
    ...
    #print("org   ", user_tran_date_dict_old)
    ...
    for userid in user_tran_date_dict.keys():
        if userid in train_userid_list:
            ...
            for basketid, basket in enumerate(train_seq):
                ...
                ##
                #org_seq = before[:-1]
                org_seq = train_seq[:basketid+1]
                for item in tar_basket:
                    T = item  
                    N = random.sample(neg_sample[userid], neg_ratio)
                    train_batch[L].append((U, S_pool, T, H_pad[0:2], N, L, org_seq))
            ...
            T_basket = before[-1]
            org_seq = train_seq[:basketid+1]
            test_batch[L].append((U, S_pool, T_basket, H_pad[0:2], L, org_seq))
            test_times += 1
            ...
            for basketid, basket in enumerate(valid_seq):
                ...
                if history == 0:
                    ...
                    valid_batch[L].append((U, S_pool, T_basket, H_pad[0:2], L, org_seq))
                    valid_times += 1
                else:
                    T_basket = before[-1]
                    valid_batch[L].append((U, S_pool, T_basket, H_pad[0:2], L, org_seq))
    ...
    for l in train_batch.keys():
        #print("tr   ", list(zip(*train_batch[l])))
...
def get_batch_TRAIN_DATASET(dataset, batch_size):
    ...
    for idx, (UU, SS, TT, HH, NN, LL, org_seq) in enumerate(dataset):
        ...
        if SS.__len__() <= batch_size:
            ...
            yield (batch_userid,batch_input_seq,batch_target,batch_history,batch_neg_items,org_seq)
        else:
            batch_begin = 0
...
def get_batch_TEST_DATASET(TEST_DATASET, batch_size):
    ...
    for idx, (UU, SS, TT_bsk, HH, LL, org_seq) in enumerate(TEST_DATASET): 
